-- VorteX Hub - Complete Edition (mobile optimized)
-- Integration: removed FPS Devourer, added Desync Body (Anti-Hit), Source (Inf Jump / JumpBoost) and FLY TO BASE
-- Main Discord updated: https://discord.gg/qvVEZt3q88 
-- Modification date: 2025-10-10
-- Mobile optimizations: descendants cache, sound pooling, adjusted intervals, optimized debounce
-- New: Body highlighting on Desync (green fill, red outline for original and clone)

----------------------------------------------------------------
-- PERSISTENCE (only BEST / SECRET / BASE)
----------------------------------------------------------------
local CONFIG_DIR = 'VorteXHub'
...
----------------------------------------------------------------
do
    local old = playerGui:FindFirstChild('VorteXHub_FULL')
    if old then
        pcall(function()
            old:Destroy()
        end)
    end
    local old3d = playerGui:FindFirstChild('VorteX3DFloor')
    if old3d then
        pcall(function()
            old3d:Destroy()
        end)
...
----------------------------------------------------------------
-- MAIN PANEL (GUI) + TOGGLES + DRAG
----------------------------------------------------------------
local gui = Instance.new('ScreenGui')
gui.Name = 'VorteXHub_FULL'
...
local espBtnY, espBtnH, espBtnW, espBtnGap = 45, 28, 0.48, 0.02
...
local btnEspBest = createButton(menu, UDim2.new(espBtnW, -4, 0, espBtnH), UDim2.new(espBtnGap, 0, 0, espBtnY), 'BEST ESP', Enum.Font.GothamBold, espConfig.enabledBest and Color3.fromRGB(0, 150, 0) or Color3.fromRGB(30, 30, 30), 6)
makePersistToggle(
    btnEspBest,
    'espBest',
...
local btnEspSecret = createButton(menu, UDim2.new(espBtnW, -4, 0, espBtnH), UDim2.new(espBtnGap * 2 + espBtnW, 4, 0, espBtnY), 'SECRET ESP', Enum.Font.GothamBold, espConfig.enabledSecret and Color3.fromRGB(200, 0, 0) or Color3.fromRGB(30, 30, 30), 6)
makePersistToggle(
    btnEspSecret,
    'espSecret',
...
local espBaseY, espBaseH = espBtnY + espBtnH + 8, 30
local btnEspBase = createButton(menu, UDim2.new(0.9, 0, 0, espBaseH), UDim2.new(0.05, 0, 0, espBaseY), 'BASE ESP', Enum.Font.GothamBold, espConfig.enabledBase and Color3.fromRGB(0, 100, 200) or Color3.fromRGB(30, 30, 30), 6)
makePersistToggle(
    btnEspBase,
    'espBase',
...
local espPlayerY = espBaseY + espBaseH + 8
local btnEspPlayer = createButton(menu, UDim2.new(0.9, 0, 0, 30), UDim2.new(0.05, 0, 0, espPlayerY), 'PLAYER ESP', Enum.Font.GothamBold, espConfig.enabledPlayer and Color3.fromRGB(200, 100, 0) or Color3.fromRGB(30, 30, 30), 6)
makeRuntimeToggle(
    btnEspPlayer,
    'enabledPlayer',
...
local aimbotY = espPlayerY + 30 + 10
local btnAimbotMain = createButton(menu, UDim2.new(0.9, 0, 0, 30), UDim2.new(0.05, 0, 0, aimbotY), 'AIMBOT', Enum.Font.GothamBold, Color3.fromRGB(200, 200, 0), 6)
btnAimbotMain.MouseButton1Click:Connect(function()
    fireWebSlingerLowerTorso()
    showStatus('Aimbot working!', Color3.fromRGB(0, 200, 0))
    local oldColor = btnAimbotMain.BackgroundColor3
    local oldText = btnAimbotMain.Text
    btnAimbotMain.BackgroundColor3 = Color3.fromRGB(0, 200, 0)
    btnAimbotMain.Text = 'ACTIVATED'
    task.wait(1)
    btnAimbotMain.Text = oldText
    btnAimbotMain.BackgroundColor3 = oldColor
end)

local btnDiscordMain = createButton(menu, UDim2.new(0.9, 0, 0, 30), UDim2.new(0.05, 0, 0, aimbotY + 30 + 10), 'DISCORD', Enum.Font.GothamBold, Color3.fromRGB(100, 0, 200), 6)
btnDiscordMain.MouseButton1Click:Connect(function()
    ...
    if copied then
        showStatus('Discord copied!', Color3.fromRGB(0, 200, 100))
        notify('Discord', 'Link copied to clipboard.', 4)
    else
        showStatus('Open: ' .. link, Color3.fromRGB(200, 200, 0))
        notify('Discord', link, 5)
    end
end)

do
    local bottom = (aimbotY + 30 + 10) + 30 + 15
    menu.Size = UDim2.new(0, 220, 0, bottom + 15)
end

----------------------------------------------------------------
-- SECONDARY PANEL: Desync / Inf Jump / Fly to Base
----------------------------------------------------------------
...
----------------------------------------------------------------
-- DESYNC / ANTI-HIT (optimized) + Highlight
----------------------------------------------------------------
...
local desyncHighlights = {}  -- To store highlights
...
local function addDesyncHighlight(model)
    ...
    highlight.FillColor = Color3.fromRGB(0, 255, 100)  -- Green fill
...
local function performDesyncLockdown(duration, onComplete)
    ...
    overlayGui.Name = 'VorteXDesyncOverlay'
    ...
    task.delay(duration, function()
        ...
        notify('Desync', 'Desync Successful!', 4)
        showStatus('Desync Successful!', Color3.fromRGB(0, 200, 0))
...
local function deactivateAntiHit()
    ...
    showNotification('Anti-Hit deactivated.', Color3.fromRGB(200, 0, 0), 2)
...
local btnDesync = createButton(panel, UDim2.new(0.88, 0, 0, 36), UDim2.new(0.06, 0, 0, 32), 'DESYNC BODY', Enum.Font.GothamBold, Color3.fromRGB(30, 30, 30), 6)

local function updateDesyncButton()
    if antiHitRunning then
        btnDesync.BackgroundColor3 = Color3.fromRGB(200, 200, 0)
    ...
    elseif antiHitActive then
        ...
        btnDesync.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
...
btnDesync.MouseButton1Click:Connect(function()
    if antiHitRunning then
        showStatus('Anti-Hit in progress...', Color3.fromRGB(200, 200, 0))
        return
    end
    if antiHitActive then
        ...
        executeAntiHit()
        updateDesyncButton()
        showStatus('DESYNC BODY ON', Color3.fromRGB(0, 200, 0))
    end
end)
...
local btnInfJump = createButton(panel, UDim2.new(0.88, 0, 0, 30), UDim2.new(0.06, 0, 0, 32 + 36 + 8), 'INFINITE JUMP', Enum.Font.GothamBold, Color3.fromRGB(30, 30, 30), 6)

local greenOn = Color3.fromRGB(0, 150, 0)
local redOff = Color3.fromRGB(30, 30, 30)
local function updateInfJumpButton()
    btnInfJump.BackgroundColor3 = sourceActive and greenOn or redOff
    btnInfJump.Text = sourceActive and 'INF JUMP ON' or 'INFINITE JUMP'
end
...
local function switchGravityJump()
    ...
    showStatus(
        ...
        sourceActive and greenOn or Color3.fromRGB(200, 200, 200)
    )
end
...
local function setFlyButtonActive(state)
    if btnFlyToBase then
        ...
        btnFlyToBase.Text = state and 'FLYING...' or 'FLY TO BASE'
    end
end
...
local function finishFly(success)
    ...
    if success then
        playFlySuccessSound()
        showStatus('Fly to Base successful!', greenOn)
    else
        showStatus('Fly to Base cancelled.', Color3.fromRGB(200, 200, 0))
    end
end
...
_G.VorteX_StartFlyToBase = startFlyToBase

updateDesyncButton()
updateInfJumpButton()

local flyY = 32 + 36 + 8 + 30 + 8
btnFlyToBase = createButton(panel, UDim2.new(0.88, 0, 0, 30), UDim2.new(0.06, 0, 0, flyY), 'FLY TO BASE', Enum.Font.GothamBold, redOff, 6)
btnFlyToBase.MouseButton1Click:Connect(function()
    startFlyToBase()
end)

----------------------------------------------------------------
-- ESP BASE (optimized with cache)
----------------------------------------------------------------
do
...
-- Transparency with backup/restoration
...
-- Creation/removal of physics bodies
...
function sfDisable()
    if not sfActive then
        -- ensure cleaning even if inactive
...
-- Respawn: always restore
sfSafeDisconnect(sfCharAddedConn)
sfCharAddedConn = player.CharacterAdded:Connect(function(ch)
    -- give time for objects to spawn
    task.wait(0.1)
    sfDisable()
end)

-- STEAL FLOOR Button
local btnStealFloor = createButton(panel, UDim2.new(0.88, 0, 0, 30), UDim2.new(0.06, 0, 0, flyY + 30 + 8), 'STEAL FLOOR', Enum.Font.GothamBold, Color3.fromRGB(30, 30, 30), 6)
btnStealFloor.MouseButton1Click:Connect(function()
    if not sfActive then
        sfEnable()
        btnStealFloor.BackgroundColor3 = Color3.fromRGB(0, 150, 0)
        btnStealFloor.Text = 'STEAL FLOOR ON'
        showStatus('Steal Floor ON', Color3.fromRGB(0, 150, 0))
    else
        sfDisable()
        btnStealFloor.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
        btnStealFloor.Text = 'STEAL FLOOR'
        showStatus('Steal Floor OFF')
    end
end)

-- Adjust panel height to include new button
do
...
openBtn.Image = 'rbxassetid://106956433392518' -- New image ID
...
glowBtn.Thickness = 2
glowBtn.Color = Color3.fromRGB(170, 0, 255)
glowBtn.Transparency = 0
...
notify('VorteX Hub', 'Script activated!', 5)

----------------------------------------------------------------
-- FINAL: Mobile optimized script
----------------------------------------------------------------
-- Optimizations applied:
-- 1. GetDescendants cache (reduces tree scanning)
-- 2. Sound pooling (limits simultaneous sounds)
-- 3. Increased intervals (1.5s ESP, 0.25s glow)
-- 4. Optimized debounce in CFrame updates
-- 5. Periodic cache cleaning
-- New: Clean black GUI with purple borders, white text, organized buttons
-- All functionalities kept intact!