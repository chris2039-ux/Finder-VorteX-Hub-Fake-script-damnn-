...
local function saveSettings()
    ...
    if not success then
        logMessage("Failed to save settings: " .. tostring(error))
    end
end
...
local function saveGUIState()
    ...
    if not success then
        logMessage("Failed to save GUI state: " .. tostring(error))
    end
end
...
local function saveAPIState()
    ...
    if not success then
        logMessage("Failed to save API state: " .. tostring(error))
    end
end
...
local function getServersFromAPI(baseUrl, isMainAPI)
    ...
    for page = 1, maxPages do
        ...
        if not successDecode or not body or not body.data then break end
...
local function tryTeleportWithRetries()
    ...
    while attempts < maxAttempts and isRunning do
        ...
        if success then
            ...
            logMessage("Teleport failed: " .. tostring(err))
...
local function runServerCheck()
    ...
    if foundPets and #results > 0 then
        ...
        showNotification("No Targets", "No high-value pets found.", 2)
    end
...
local function createSettingsGUI()
    ...
    local function startHopping()
        ...
        showNotification("VorteX Finder", "Search started! Looking for high-value pets...", 3)
    ...
    stopButton.MouseButton1Click:Connect(function()
        ...
        showNotification("VorteX Finder", "Search stopped.", 2)
    end)
