...
-- [VORTEX MOD] Cambiados los nombres de archivo
...
-- [VORTEX MOD] Tema morado oscuro mejorado
local THEME = {
...
-- [VORTEX MOD] Función de notificación mejorada
    ...
    for i = 1, #plotChildren do
        ...
        if not isPlayerBase(plot) then
            ...
            for _, obj in ipairs(descendants) do
                if obj:IsA("Attachment") and (obj.Name == "OVERHEAD_ATTACHMENT" or obj.Name == "Attachment") then
...
local function tryTeleportWithRetries()
    ...
    while attempts < maxAttempts and isRunning do
        ...
        if #servers == 0 then
            logMessage("No servers available, retrying...", THEME.Warning)
    ...
    if isRunning then
        logMessage("Max teleport attempts reached", THEME.Error)
        isRunning = false
    end
end
...
local function monitorFoundPodiums()
    ...
    monitoringConnection = RunService.Heartbeat:Connect(function()
        ...
        if lostAny then
            ...
            showNotification("Target Lost", lostText, 3)
        end
    end)
end
...
local function runServerCheck()
    ...
    if foundPets and #results > 0 then
        ...
        logMessage("Found " .. #results .. " target(s): " .. foundText, THEME.Success)
    ...
    logMessage("Hop #" .. settings.hopCount .. " - Checking for servers...", THEME.Text)
    saveSettings()
...
local function createTagList(parent, list, placeholder, onAdd, onRemove)
    ...
    local function updateCanvas()
        ...
        scrollFrame.CanvasSize = UDim2.new(0, math.max(totalWidth, scrollFrame.AbsoluteSize.X), 0, 0)
    end
...
local function createSettingsGUI()
    ...
    screenGui.Name = "VorteXFinderGUI" -- [VORTEX MOD] Nombre cambiado
    ...
    titleLabel.Text = "VorteX Finder" -- [VORTEX MOD] Nombre cambiado
    ...
    local function startHopping()
        ...
        logMessage("VorteX Finder started", THEME.Success)
        showNotification("VorteX Finder", "Searching for target pets...", 3)
        ...
        hopConnection = task.spawn(function()
            while isRunning do
                ...
                task.wait(0.5) -- Aumentado el tiempo de espera para reducir carga
                updateAPIStatus()
            end
        end)
    end
...
if game.PlaceId == ALLOWED_PLACE_ID then
    task.wait(1)
    createSettingsGUI()
    logMessage("VorteX Finder initialized successfully", THEME.Success)
    showNotification("VorteX Finder", "Ready to search for pets!", 3)
else
