...
local joinOptionsShown = false  -- Nueva variable para controlar si ya se mostró el panel
...
local function showJoinOptions(podiumsData)
    ...
    optionGui.DisplayOrder = 999  -- Asegurar que esté encima
    optionGui.Parent = playerGui
    
    local mainFrame = Instance.new("Frame")
    mainFrame.Size = UDim2.new(0, 400, 0, 250)  -- Tamaño aumentado
    ...
    descLabel.Size = UDim2.new(1, -30, 0, 120)
...
local function runServerCheck()
    ...
    if foundPets and #results > 0 then
        ...
        -- Mostrar opciones de join (SOLO SI autoJoin está desactivado)
        if not settings.autoJoin then
            showJoinOptions(results)
        else
            -- Si autoJoin está activado, quedarse automáticamente
            logMessage("Auto-join enabled, staying in server", THEME.Success)
